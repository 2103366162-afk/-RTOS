...
// void os_task_slice_test (void) {    
//     os_err_t err;
    
// 	err = os_task_init(&task1, "task1", task1_entry, OS_NULL, 0, task1_stack, sizeof(task1_stack));
// 	os_assert(err == OS_ERR_OK);
// 	os_task_start(&task1);
	
// 	err = os_task_init(&task2, "task2", task2_entry, OS_NULL, 0, task2_stack, sizeof(task2_stack));
// 	os_assert(err == OS_ERR_OK);
// 	os_task_start(&task2);

// 	err = os_task_init(&task3, "task3", task3_entry, OS_NULL, 1, task3_stack, sizeof(task3_stack));
// 	os_assert(err == OS_ERR_OK);
// 	os_task_start(&task3);

// 	err = os_task_init(&task4, "task4", task4_entry, OS_NULL, 1, task4_stack, sizeof(task4_stack));
// 	os_assert(err == OS_ERR_OK);
// 	os_task_start(&task4);
// }
