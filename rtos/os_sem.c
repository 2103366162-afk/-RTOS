...
os_err_t os_sem_release (os_sem_t * sem)
{
    ...
    /* 中断中禁止调用，请使用 os_sem_release_from_isr */
    if (os_core.os_sched_in_isr_flag > 0) 
    {
 ...
 *
 * @brief   中断中尝试获取信号量（非阻塞）
 *          如果信号量计数 >0，则减一返回 OK；
 *          否则立即返回错误，不会等待。
 *          不能在中断中调用会阻塞的 os_sem_take。
 *
 * @return  OS_ERR_OK    获取成功
 *          OS_ERR_SEM_CNT_ZERO  当前计数为0，获取失败
 */
 ...
 *
 * @brief   中断中释放信号量（非阻塞）
 *          如果有任务在等待，则唤醒一个任务（但不立即切换，切换由
 *          os_isr_leave() 触发）；否则计数+1。
 *
 * @return  OS_ERR_OK              成功
 *          OS_ERR_SEM_CNT_OUT     计数已满，无法释放
 */
os_err_t os_sem_release_from_isr (os_sem_t * sem)
{
    ...
    if (cnt > 0) {
        ...
        /* 注意：这里不能调用 os_sched_run()，因为 ISR 切换统一由 os_isr_leave() 处理 */
        os_sched_isr_enable(status);
        return OS_ERR_OK;
    }
